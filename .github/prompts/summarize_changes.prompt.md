...
- 検証手順（必須）: 出力を生成する前に、以下の検証を行ってください。
  1. `path` が `agents/` または `skills/` を参照する場合、そのファイルが実際にリポジトリに存在することを確認してください。
  2. 既存の `summaries/agent_en.md`（agents）または `summaries/skill_en.md`（skills）に該当ファイルの要約が含まれているかを確認してください。
  3. 要約が存在しない場合は、`summary` フィールドに frontmatter の `description` か本文の最初の段落を用いて自然な日本語要約を生成し、呼び出し元のワークフローに戻すだけでなく、可能であれば `summaries/` へ追記するための行（agents なら `### date — path (N)` ブロック、skills ならテーブル行）も出力してください。
  4. 要約が存在するが内容が古い／不一致と判断した場合は、その旨を短く注記して更新用の `summary` を返してください。

 これにより、プロンプト実行時に検出と簡易修正の指示が同時に出力され、後続の自動化（スクリプト適用／コミット）に繋げやすくなります。
